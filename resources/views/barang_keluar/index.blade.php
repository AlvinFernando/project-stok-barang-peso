                                            ...
                                            <div class="row">
                                                <div class="col-sm-12">
                                                    <table id="example2" class="table table-bordered table-hover dataTable dtr-inline" role="grid" aria-describedby="example2_info">
                                                        ...
                                                        <tbody>
                                                            @forelse($barangkeluars as $result => $brgs)
                                                                <tr>
                                                                    <td>{{ $result + $barangkeluars->firstItem() }}</td>
                                                                    <td>{{ $brgs->barangs['item_description'] }}</td>
                                                                    <td>{{ $brgs->customer }}</td>
                                                                    <td>{{ $brgs->qty }}</td>
                                                                    <td>{{ $brgs->tanggal }}</td>
                                                                    <td>{{ $brgs->keterangan }}</td>
                                                                    <td>
                                                                        <form action="{{ route('barang_keluar.destroy', $brgs->id )}}" method="POST">
                                                                            @csrf
                                                                            @method('delete')
                                                                            <a href="{{ route('barang_keluar.show', $brgs->id) }}" class="btn btn-info btn-sm"><i class="fas fa-eye nav-icon text-light"></i></a>
                                                                            <a href="{{ route('barang_keluar.edit', $brgs->id) }}" class="btn btn-warning btn-sm"><i class="fas fa-edit nav-icon text-light"></i></a>
